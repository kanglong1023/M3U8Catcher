...
import com.kanglong.m3u8.core.M3u8Resolver.MediaSegment;
...
import static com.kanglong.m3u8.core.M3u8Download.m3u8StoreName;
import static com.kanglong.m3u8.core.M3u8Download.unFinishedTsExtension;
...
public class TsDownloadPlanner {
    ...
    public List<TsDownload> plan() {
        ...
        // convert
        List<TsDownload> tsDownloads = convertToTsDownloads(START_OVER == optionsForApplyTsCache, tsDir, mediaSegments, secretKeyMap);
    ...
    private void checkTsCache(String identity, Path tsDir, Path m3u8StorePath,
        ...
        if (START_OVER == optionsForApplyTsCache) {
            log.info("start download all over: {}", identity);
            return;
        }

        if (FORCE_APPLY_CACHE_BASED_ON_FILENAME == optionsForApplyTsCache) {
            log.info("start download force apply cache based on filename: {}", identity);
            return;
        }

        if (SANITY_CHECK != optionsForApplyTsCache) {
            return;
        }
